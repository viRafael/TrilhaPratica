...
INSERT INTO Queries (QueryNumero, QueryTexto) VALUES
    (1, 'SELECT * FROM PRODUTO WHERE cd_ean_prod = ''25'';'),
    ...
    (12, 'SELECT * FROM Fornecedor_Produto WHERE ce_cod_forn = 10;'),
    (13, 'SELECT * FROM ESTABELECIMENTO WHERE cidade_estab = ''São Paulo'' AND tipo = ''Deposito'';'),
    (14, 'SELECT * FROM ESTABELECIMENTO WHERE cidade_estab = ''São Paulo'' AND tipo = ''Deposito'';'),
    ...
    (29, 'SELECT f.* FROM FUNCIONARIOS f JOIN PRODUTO p ON f.cp_cod_func = p.ce_cod_func JOIN ESTABELECIMENTO e ON p.ce_cod_estab = e.cp_cod_estab WHERE e.tipo = ''Loja'';'),
    ...
    (32, 'SELECT f.cnpj_forn, COUNT(DISTINCT p.ce_cod_estab) AS total_estabelecimentos FROM FORNECEDOR f JOIN Fornecedor_Produto fp ON f.cp_cod_forn = fp.ce_cod_forn JOIN PRODUTO p ON fp.ce_id_produto = p.cp_id_produto GROUP BY f.cnpj_forn ORDER BY total_estabelecimentos DESC LIMIT 1;'),
    ...
    (38, 'SELECT f.* FROM FORNECEDOR f WHERE NOT EXISTS (SELECT 1 FROM Fornecedor_Produto fp WHERE fp.ce_cod_forn = f.cp_cod_forn AND fp.dt_vencimento < CURRENT_DATE::TEXT);'),
    (38, 'SELECT f.* FROM FORNECEDOR f WHERE NOT EXISTS (SELECT 1 FROM Fornecedor_Produto fp WHERE fp.ce_cod_forn = f.cp_cod_forn AND fp.dt_vencimento < CURRENT_DATE::TEXT);'),
    ...
    (42, 'SELECT c.* FROM CATEGORIA c WHERE NOT EXISTS (SELECT 1 FROM PRODUTO p JOIN Fornecedor_Produto fp ON p.cp_id_produto = fp.ce_id_produto JOIN FORNECEDOR f ON fp.ce_cod_forn = f.cp_cod_forn WHERE p.ce_categoria_principal = c.cp_cod_categoria AND f.cidade_forn = ''São Paulo'');'),
